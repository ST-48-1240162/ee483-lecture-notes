...
\begin{examplebox}[title={Example (Computing the Convolution Sum)}, handout mark=2, breakable]
...
\begin{handouttikz}[x=1.1cm, y=0.62cm]
  \tikzset{
    cell/.style={draw, minimum width=0.78cm, minimum height=0.52cm, font=\small},
    lbl/.style={font=\small, anchor=east}
  }

  \node[font=\footnotesize, align=left, anchor=west] at (-0.2,2.05)
    {Flip $h[n]$ to $g[m]=h[-m]$; align $x[m]$ and $g[m]$. At $n=0$:};
...
\begin{defbox}[title={BIBO Stability\HandoutTitleFnmark}]
...
\end{equation}
\end{defbox}
\footnotetext{cf. Mitra Sec.~4.4.3.}
